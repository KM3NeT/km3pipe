...
class CheckRecoContents(kp.Module):
    ...
    def process(self, blob):
        ...
        all_tracks_raw_jgandalf_chi2 = all_tracks_raw_fitinf[:,km3io.definitions.fitparameters.JGANDALF_CHI2]
        
        #best tracks
        best_tracks_raw = km3io.tools.best_jmuon(all_tracks_raw)
        best_tracks_raw_dir_z = best_tracks_raw.dir_z
        best_tracks_raw_jgandalf_chi2 = best_tracks_raw.fitinf[km3io.definitions.fitparameters.JGANDALF_CHI2]
        ...
        for i in range(len(all_tracks_raw_dir_z)):
            ...
            if not np.isnan(all_tracks_raw_jgandalf_chi2[i]) and not np.isnan(tracks_jgandalf_chi2[i]):
                assert np.allclose(all_tracks_raw_jgandalf_chi2[i],tracks_jgandalf_chi2[i])
